...
FROM cloud_files(
    '/Volumes/dlt_cat/db09/landingzone/customers/', 
    'csv', 
...
FROM cloud_files("/Volumes/dlt_cat/db09/landingzone/invoices/", "csv", map("cloudFiles.inferColumnTypes", "true"))
...
-- DBTITLE 1,Multi table names is not supported
CREATE LIVE TABLE dlt_cat.db09.cust_daily_sales_uk_2022
AS
with daily_sales as (
...
FROM LIVE.invoices
WHERE invoice_year = 2022 AND country="United Kingdom"
GROUP BY country, invoice_year, invoice_month, invoice_date, customer_id
) 
,  cust as ( select customer_id, customer_name FROM LIVE.customers 
  where _end_at IS NULL)
  SELECT country, invoice_year, invoice_month, invoice_date,
          round(sum(quantity*unit_price),2) as total_sales,
          customer_name
        FROM daily_sales s 
        INNER JOIN cust cus ON s.customer_id = cus.customer_id
